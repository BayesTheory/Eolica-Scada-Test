"""
Motor de Treinamento Unificado (v3.1)
- Usa constantes para nomes de modelos registrados.
- Recebe o nome do experimento para garantir o isolamento das tarefas.
- Assegura que o registro do modelo seja robusto contra falhas de conexão.
"""
import pandas as pd
import numpy as np
import torch
...
# ==============================================================================
def run_pytorch_cv(model_class, model_key, config, input_window_size, output_horizon_size, cv_splits, experiment_name):
    ...
    with mlflow.start_run(run_name=run_name) as parent_run:
        ...
        for fold, (train_index, test_index) in enumerate(tscv.split(X_tensor)):
            run_name_fold = f"{model_key}_fold_{fold+1}"
            with mlflow.start_run(run_name=run_name_fold, nested=True) as child_run:
        ...
        try:
            ...
            best_run_df = mlflow.search_runs(experiment_ids=[exp_id], filter_string=f"tags.'mlflow.parentRunId' = '{parent_run.info.run_id}'", order_by=["metrics.rmse_kW ASC"], max_results=1)
            if not best_run_df.empty:
                ...
                print(f"\nMelhor modelo (Run ID: {best_run_id}) registrado com sucesso como '{POWER_FORECASTER_MODEL_NAME}'!")
        except Exception as e:
            print(f"\nAVISO: Falha ao registrar o melhor modelo. Verifique a conexão com o MLflow.")
            print(f"Detalhe do erro: {e}")

# ==============================================================================
# MOTOR DE TREINAMENTO PARA MODELOS ESTATÍSTICOS (ARIMA)
# ==============================================================================
def run_arima_cv(model_class, model_key, config, output_horizon_size, cv_splits, experiment_name):
    ...
    with mlflow.start_run(run_name=run_name) as parent_run:
        ...
        for fold, (train_index, test_index) in enumerate(tscv.split(time_series)):
            # Esta função não usa runs aninhados, loga as métricas do fold no run pai
            print(f"\n--- Iniciando Fold {fold+1}/{cv_splits} ---")
            ...
            fold_metrics['rmse'].append(rmse); fold_metrics['mae'].append(mae); fold_metrics['r2'].append(r2)
            # Logando métricas com passo para diferenciá-las no MLflow
...
# ==============================================================================
# MOTOR DE TREINAMENTO PARA MODELOS DE ÁRVORE (XGBOOST)
# ==============================================================================
def _create_lagged_features_corrected(df, features_to_lag, target_col, n_lags):
    """Função auxiliar para criar features de lag sem vazamento de dados."""
...
def run_xgboost_cv(model_class, model_key, config, cv_splits, experiment_name):
