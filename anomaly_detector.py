"""
Anomaly Detector (v2.1) - Orquestrador Integrado
...
MLFLOW_TRACKING_URI = "http://1227.0.0.1:5000"

# Limiares de Anomalia
PERFORMANCE_ANOMALY_THRESHOLD_STD = 3.0
FAILURE_PROBABILITY_THRESHOLD = 0.75

def _load_model_artifacts(model_name, stage):
    """Função auxiliar para carregar um modelo e seus artefatos."""
    ...
    # Carrega scaler de features, que sempre deve existir
    with open(f"{local_path}/scaler_features.pkl", "rb") as f:
        scaler_features = pickle.load(f)
    
    # Carrega scaler de alvo, que pode não existir (ex: para o modelo de falha)
    scaler_target = None
...
def load_models():
    ...
    try:
        power_model_artifacts = _load_model_artifacts(POWER_MODEL_NAME, MODEL_STAGE)
        fault_model_artifacts = _load_model_artifacts(FAULT_MODEL_NAME, MODEL_STAGE)
        print("Modelos e artefatos carregados com sucesso.")
        return power_model_artifacts, fault_model_artifacts
...
def check_performance_anomalies(df, artifacts):
    ...
    features = config['MODELOS']['3GRU']['features']
    ...
    error_threshold = np.std(error) * PERFORMANCE_ANOMALY_THRESHOLD_STD
...
def main():
    ...
    power_model_artifacts, fault_model_artifacts = load_models()
    
    if power_model_artifacts and fault_model_artifacts:
        ...
        perf_anomalies = check_performance_anomalies(df_features, power_model_artifacts)
        imminent_failure_risk = predict_imminent_failures(df_features, fault_model_artifacts)
